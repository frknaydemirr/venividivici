...
async def get_city(request: Request, city_id: int):
    city = await asyncio.to_thread(request.app.ctx.db.get_specific_city, city_id=city_id)
...
async def get_cities_in_specific_country(request: Request, country_id: int):
    ...
    cities = await asyncio.to_thread(request.app.ctx.db.get_cities_in_specific_country, country_id=country_id, offset=offset, limit=limit)
...
async def get_most_conquered_cities(request: Request):
    ...
    cities = await asyncio.to_thread(request.app.ctx.db.get_most_conquered_cities, limit)
